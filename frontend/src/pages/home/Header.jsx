...
function Header() {
    ...
    const handleJoin = async (e) => {
        ...
        try {
            const res = await axios.post(`http://localhost:3002/api/match/${inputValue}/join`, {}, {
                headers: {
                    "leetcode-session": Cookies.get("LEETCODE_SESSION"),
                    "csrf-token": Cookies.get("CSRF_TOKEN"),
                }
            });

            console.log(inputValue);
            location.href = `/match/${inputValue}/play`;
        } catch (e) {
            console.log(e);
            alert(`failed to join lobby, ${e.response.data.error}`);
        }
    ...
    const handleCreateLobby = async (e) => {
        ...
        try {
            const res = await axios.post("http://localhost:3002/api/match/create", {}, {
                headers: {
                    "leetcode-session": Cookies.get("LEETCODE_SESSION"),
                    "csrf-token": Cookies.get("CSRF_TOKEN"),
                }
            ...
            location.href = `/match/${res.data.matchCode}/play`;
        } catch (e) {
            console.log(e);
        }
                ...
                <div className='relative justify-center align-baseline flex w-full '>
                    ...
                    <div style={{ backgroundImage: `url(${WoodBg})` }} className='relative bg-wood3 w-2/3 flex-col justify-center items-center p-10 rounded-md border-wood2 border-8 drop-shadow-2xl flex '>
                        ...
                        <div className='font-Montserrat'>
                            <form onSubmit={handleJoin} className="flex items-center justify-center space-x-1 p-4">
                                <input type="text"
                                    value={inputValue}
